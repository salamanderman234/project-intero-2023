...
type ClassSubjectEntity struct {
	...
	Teacher 	map[string]any 	`json:"teacher"`
	ClassID 	*uint 			`json:"class_id,omitempty" gorm:"not null" valid:"int~class id field must be an integer,required~class id field is required"`
...
type ClassSubjectRepository interface {
	...
	Read(c context.Context, id uint, studentID uint, teacherID uint, classID uint, year uint, orderBy string, orderWith string) ([]ClassSubjectModel, uint, error)
...
type ClassSubjectService interface {
	...
	UpdateClassSubject(c context.Context, id uint, data ClassSubjectUpdateForm) (ClassSubjectEntity, error)
